                        ...
                        for (i=0; i < app.engine.screen.tilesX; i++) {
                            ...
                            for (var l = 0; l < len; l++) {
                                ...
                                if (npc.x == mapX && npc.y == mapY) {
                                    ...
                                    var npc_name = app.engine.tilesets.descriptors.characters[npc.id].name;
                                    if (redrawNametags) app.engine.nametags.add(npc_name, i, j);
                                    app.engine.tile.drawPlayer(i, j, index, npc.id);
                                }
                            }

                            if (app.engine.map.corruptionDataLoaded && mapX >= 0 && mapX < app.engine.TOTALTILES_X && mapY >= 0 && mapY < app.engine.TOTALTILES_Y && app.engine.map.corruption[mapX][mapY] === 1) {
